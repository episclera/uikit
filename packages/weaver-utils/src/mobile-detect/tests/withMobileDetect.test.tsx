...
describe('withMobileDetect', () => {
  it('should detect correct device from window navigator user-agent if no request provided', async () => {
    ...
    const checkMobileRes = await withMobileDetectComponent.getInitialProps({})
    expect(checkMobileRes.deviceContext.isMobile).toBeTruthy()
    ...
    const checkTabletRes = await withMobileDetectComponent.getInitialProps({})
    expect(checkTabletRes.deviceContext.isTablet).toBeTruthy()
    ...
    const checkDesktopRes = await withMobileDetectComponent.getInitialProps({})
    expect(checkDesktopRes.deviceContext.isDesktop).toBeTruthy()
  })

  it('should detect correct device from provided (different request format) user agent header', async () => {
    const withMobileDetectComponent = withMobileDetect(TestComponent)
  ...
  it('should correct re-implement getInitialProps method and merge component props with device detect props', async () => {
    const withMobileDetectComponent = withMobileDetect(TestComponent)

    const props = await withMobileDetectComponent.getInitialProps({
      req: {
        headers: {
          'user-agent': userAgents.chromeMobile,
        },
